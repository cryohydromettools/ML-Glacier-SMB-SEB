...
mb_cosipy = ds_cosipy["MB"].sel(time=t_slice).sum("time")
mb_xgb    = ds_xgb["SMB_mean"].sel(time=t_slice).sum("time")
mb_nn     = ds_nn["SMB_mean"].sel(time=t_slice).sum("time")
...
p0 = axes[0, 0].contourf(
    ...
    cmap="RdBu", levels=smb_levels, norm=norm_smb, extend="both"
)
axes[0, 0].set_title("COSIPY SMB (2022)", fontsize=title_fs)
...
axes[0, 1].set_title("XGB SMB (2022)", fontsize=title_fs)

p2 = axes[0, 2].contourf(
    mb_nn.lon, mb_nn.lat, mb_nn.where(mask==1),
    cmap="RdBu", levels=smb_levels, norm=norm_smb, extend="both"
)
axes[0, 2].set_title("NN SMB (2022)", fontsize=title_fs)
...
axes[1, 1].set_title("COSIPY − NN", fontsize=title_fs)

p5 = axes[1, 2].pcolormesh(
    diff_xgb_nn.lon, diff_xgb_nn.lat, diff_xgb_nn,
    cmap="bwr", norm=norm_diff
)
axes[1, 2].set_title("XGB − NN", fontsize=title_fs)
...
cbar_smb.set_label("SMB (m w.e.)", fontsize=cbar_fs)
...
cbar_diff.set_label("Δ SMB (m w.e.)", fontsize=cbar_fs)
...
fig.savefig(
    dir_figures / "SMB_Arteson_2022_3x2_COSIPY_XGB_NN.png",
    dpi=300,
    facecolor="w",
    bbox_inches="tight"
)
